...
class _ManagerSumTabState extends State<ManagerSumTab>
  ...
  Future<void> _deleteSum(
      ClientModel clientModel, List<SumModel> sums, int index) async {
    showDialog(
        context: context,
        builder: (context) {
          return AlertDialog(
            title: ListTile(
              title: Text("Mijoz: ${clientModel.client_name}"),
              subtitle: Text("Summa: ${sums[index].sum!.sum}"),
            ),
            ...
            actions: [
              ...
              ElevatedButton(
                  onPressed: () {
                    Navigator.of(context).pop();
                    sumCollection.doc(sums[index].id).delete();
                  },
                  child: const Text("Ha"))
            ],
          );
  ...
  Future<void> _updateSum(SumModel product) async {
    sumCollection.doc(product.id).update({
      "sum":
          Price(sum: product.sum!.sum ?? 0.0, currency: product.sum!.currency)
              .toMap(),
