...
class ConfirmPasswordPage extends StatefulWidget {
  ...
  const ConfirmPasswordPage({super.key, required this.password});
...
class _ConfirmPasswordPageState extends State<ConfirmPasswordPage> {
    ...
    _passwordController.addListener(() {
      if (_passwordController.text.trim().length == 6) {
        if (_passwordController.text == widget.password) {
          GetStorage()
              .write("password", _passwordController.text.trim())
              .then((value) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
              content: Text("Parollar mos kelmadi. Qayta tekshirib ko'ring")));
        }
      }
    });
    super.initState();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        appBar: AppBar(
          title: const Text("Parolni tasdiqlang"),
        ),
        body: Column(
